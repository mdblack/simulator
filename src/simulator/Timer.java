...
//		System.out.printf("Writing %x to timer at address %x\n",data,address);
    ...
    public class Channel implements Clock.ClockedDevice {
        ...
        public void loadState(String state) {
            ...
            countValue = s.nextInt();
            outputLatch = s.nextInt();
            inputLatch = s.nextInt();
            countLatched = s.nextInt();
            readState = s.nextInt();
            writeState = s.nextInt();
            rwMode = s.nextInt();
            mode = s.nextInt();
            bcd = s.nextInt();
            statusLatched = s.nextInt() == 1;
            nullCount = s.nextInt() == 1;
            countStartTime = s.nextLong();
            nextTransitionTimeValue = s.nextLong();
            irq = s.nextInt();
            id = s.nextInt();
            enabled = s.nextInt() == 1;
            expireTime = s.nextLong();
        }
